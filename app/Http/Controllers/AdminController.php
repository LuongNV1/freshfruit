...
class AdminController extends Controller
{
    ...
    public function show_dashboard(Request $request){
    	...
    	return view('admin.dashboard')->with(compact('level','visiter_tuan_count','visiter_thang_count','visiter_count','visiter_total_count','product_donut','post_donut','order_donut','customer_donut','category_donut','brand_donut','post_view','product_view','post_new'));
    }
    public function dashboard(Request $request){
    	$admin_name = $request->admin_name;
    	$admin_password = md5($request->admin_password);
    	$result = DB::table('tbl_admin')->where('admin_name',$admin_name)->where('admin_password',$admin_password)->first();
    	if ($result && $result->level==10) {
